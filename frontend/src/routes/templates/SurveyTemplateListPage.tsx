...
export function SurveyTemplateListPage({ variant = 'manage' }: { variant?: 'manage' | 'pick' }) {
  ...
  return (
    <div className="page">
      ...
      {variant === 'manage' && <p className="muted">Click a template to start a new survey from it.</p>}
          ...
          {sortByDate(templates, sort).map((t) => {
            ...
            return (
              <li key={t.id} className={canManage ? 'has-corner-action' : undefined}>
                ...
                {canManage && (
                  <Link to={`/surveys/${t.id}/edit`} className="corner-link" title="Manage template">
                    Manage template
                  </Link>
                )}
              </li>
